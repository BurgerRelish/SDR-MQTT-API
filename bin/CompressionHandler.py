...
class CompressionHandler:
    ...
    def decompress_message(self, client_id: str, topic: str, message: str):
        with self.executor.submit(self.brotli_decompress, args = [client_id, topic, message]) as future:
            future.add_done_callback(self.handle_decompressed_message)
            
    def compress_message(self, client_id: str, topic: str, message: str):
        with self.executor.submit(self.brotli_compress, args = [client_id, topic, message]) as future:
            future.add_done_callback(self.handle_compressed_message)

    def handle_compressed_message(self, future):
        result = future.result()
        self.compression_callback(result[0], result[1], result[2])
    
    def handle_decompressed_message(self, future):
        result = future.result()
        self.decompression_callback(result[0], result[1], result[2])
    ...
    def brotli_compress(self, client_id: str, topic: str, message: str) -> List[str, str]:
        retval = {}
        retval['enc'] = "br"
        ...
        return [client_id, topic, json.dumps(retval)]
    ...
    def brotli_decompress(self, client_id: str, topic: str, message: str) -> List[str, Any]:
        ...
        try:
            ...
            return [client_id, topic, json.loads(str())]
